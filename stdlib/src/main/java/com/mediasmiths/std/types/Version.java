...
public class Version implements Comparable<Version>, Serializable {
	 ...
	 * 
	 * @param versionString String The version string (eg. "1.2.3"). it MUST have at least ONE digit and match the regex ^(min|max|([0-9]+\.)*[0-9]+(\.(max|min))?)$
	 */
				...
				if (sVersion[i].toLowerCase().equals("min")) {
					...
					if (!minmaxSet) {
						...
						throw new NumberFormatException(
								"A Version string may only have min or max at the end. It must comply with the following regex: ^((min|max)|([0-9]+(\\.[0-9]+)*(\\.min|\\.max)?))$");
					}
				}
			}
		}
	 ...
	 * 
	 * @param index int The index to retrieve (based at 0)
	 * @param defaultValue int The default value to return if the index doesn't exist
	 ...
	 * 
	 * @param min1 Version The 1st range
	 * @param max1 Version The 1st range
	 * @param min2 Version The 2nd range
	 * @param max2 Version The 2nd range
	...
	public String toString() {
		...
		return SpeedyString.fastJoin(".", segments);
	}
	...
	/**
	 * Produces a fairly unreliable hashCode (eg. 1.0 and 1.0.0 are identical but 1.2 and 2.1 are also identical) by adding the version bits together
	 */
	@Override
	public int hashCode() {
		int total = 0;
	...
	/**
	 * @deprecated DO NOT USE THIS METHOD - FOR BEAN SERIALISER ONLY
	 * @param content int[] A version as an integer array representing the segments (where Integer.MAX_VALUE means "max")
	 */
	@Deprecated
