...
public class StreamUtil {
	 ...
	 * 
	 * @param is InputStream The input stream to read to the end of
	...
	public static long eatInputStream(InputStream is) {
		try {
			...
			while (avail > 0) {
				...
				if (avail < CHUNKSIZE) { // If the buffer wasn't full, wait a short amount of time to let it fill up
	...
	public static void doBackgroundCopy(
	...
	public static void streamCopy(final InputStream in, final OutputStream out, final ICopyProgressMonitor monitor) {
		...
		try {
			...
			while (true) {
				...
				if (numbytes < CHUNKSIZE) { // If the buffer wasn't full, wait a short amount of time to let it fill up
					try {
						Thread.sleep(STREAM_SLEEP_TIME);
					}
					catch (InterruptedException e) {
