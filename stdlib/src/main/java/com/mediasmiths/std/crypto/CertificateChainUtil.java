package com.mediasmiths.std.crypto;
...
public class CertificateChainUtil {
	 ...
	 * 
	 * @param chain a normalised chain
	 * @return
	 */
		...
		// Keep going until we find a root certificate (or until the chain can't be continued)
		{
			...
			while (current != null && (old == null || !old.equals(current)) && !isSelfSigned(current)) {
				...
				if (current != null)
					...
					log.warn("{buildChainFor} Building chain for " + certs.size() + " cert[s] but had to stop after " + chain.size() +
							" because I could not find the issuer for " + old.getSubjectX500Principal());
					throw new IllegalArgumentException("Could not determine issuer for certificate: " + old.getSubjectX500Principal() +
	 ...
	 * 
	 * @param chain a certificate chain with the root certificate first
	 * @throws IllegalArgumentException if the chain is invalid, null or empty
	 */
	public static void verifyChain(List<X509Certificate> chain) {
		...
		for (int i = 0; i < chain.size(); i++) {
			final X509Certificate certificate = chain.get(i);
			final int issuerIndex = (i != 0) ? i - 1 : 0; // The index of the issuer is the previous cert (& the root must, of course, sign itself)
