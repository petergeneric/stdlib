 ...
 * Internally the amount of storage this object refers to is stored in bits. Any time getAmount() or getBytes() is called this value is converted to the necessary unit<br />
 * 
 * Users are discouraged from using <code>getAmount()</code> unless a rounded value is acceptable (which it rarely is). In most circumstances users are recommended to use <code>getDecimalAmount()</code>
 */
public final class StorageSize implements Comparable<StorageSize>, Serializable {
	 ...
	 * If the unit is in BITS then any fractional bytes resulting from the conversion will be discarded (e.g. passing 9 bits will result in 1 byte)
	 * 
	 * @param amount the amount component of the space this StorageSize should represent
	 * @param unit the unit that <code>amount</code> is expressed in
	 */
	 ...
	 * Since this is an approximation some information may be lost. This method converts the amount to a number of bytes, then converts it to an integer using the toBigInteger() method of BigDecimal, so information loss should be minimised
	 * 
	 * @param amount the amount component of the space this StorageSize should represent
	 * @param unit the unit that <code>amount</code> is expressed in. If the unit is <code>BITS</code> then any fractional bits will be ignored (i.e. identical to calling the BigInteger constructor with <code>amount.toBigInteger()</code>)
	 */
	 ...
	 * 
	 * @param unit the unit the user would like to see their StorageSize in by default
	 * @param bytes the number of <strong>bytes</strong> this StorageSize represents
	 */
	 ...
	 * This method may result in information being lost (e.g. partial bits). Users are encouraged to use the <code>getBits</code> method if this loss is unacceptable
	 * 
	 * @return the number of bytes represented by this object (determined by diving the number of bits by 8 and then rounding the result)
	 */
	 ...
	 * The calculations may result in minimal loss of precision, however this is unavoidable.
	 * 
	 * @param unit the unit to use (instead of the default StorageSize unit)
	 ...
	 * Uses the default representation unit for this type<br />
	 * This method may result in the loss of information (due to rounding). If precision is required then getDecimalAmount() should be used<br />
	 * 
	 * 
	...
	/**
	 * Converts the size of this StorageSize to another unit, rounding the result to an integer<br />
	 * This method may result in the loss of information (due to rounding). If precision is required then getDecimalAmount(unit) should be used<br />
	 * 
	 * @param unit the unit to use (instead of the default StorageSize unit)
	 ...
	 * 
	 * @param unit the unit to represent the amount in
	 * 
	 * @return a short ISO String version of this size using the provided unit (e.g. "4 MiB")
	 */
	 ...
	 * Since this class has a natural ordering that is inconsistent with equals, this method may return 0 where <code>equals</code> would not return <code>0</code> - since this compares based on bits but equals tests based on the unit+amount
	 * 
	 * @param that some other storage amount
	 ...
	 * Since this class has a natural ordering that is inconsistent with equals, this method may return 0 where <code>equals</code> would not return <code>0</code> - since this compares based on bits but equals tests based on the unit+amount
	 * 
	 * @param amount the number of units for this amount
	 * @param unit the unit that amount should be expressed in
	 * @see java.lang.Comparable#compareTo(java.lang.Object)
	 */
