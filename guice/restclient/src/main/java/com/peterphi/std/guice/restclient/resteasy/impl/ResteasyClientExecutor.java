...
public class ResteasyClientExecutor implements ClientHttpEngine {
    private final static Logger logger = Logger.getLogger(ClientHttpEngine.class);
    ...
    public ResteasyClientExecutor(HttpClient httpClient) {
        this.httpClient = httpClient;
    }

    public ResteasyClientExecutor(HttpClient httpClient, boolean closeHttpClient) {
        this.httpClient = httpClient;
        this.createdHttpClient = closeHttpClient;
    }


    public ResteasyClientExecutor(HttpClient httpClient, HttpContext httpContext) {
        this.httpClient = httpClient;
    ...
    public void setDefaultProxy(HttpHost defaultProxy) {
        //Not sure how to update the RequestConfig of an existing httpClient
        throw new NotImplementedException("setDefaultProxy is not currently supported");
    }
    ...
    protected HttpRequestBase createHttpMethod(String url, String restVerb) {
        ...
        if ("GET".equals(restVerb)) {
            ...
            return httpRequest;
        } else if ("PUT".equalsIgnoreCase(restVerb)) {
            httpRequest = new HttpPut(url);
            return httpRequest;
        } else if ("DELETE".equalsIgnoreCase(restVerb)) {
    ...
    protected void loadHttpMethod(final ClientInvocation request, HttpRequestBase httpMethod) throws Exception {
        //Because RequestConfig is gathered from only one place this is required
        ...
        RequestConfig.Builder requestBuilder = RequestConfig.copy(baseConfig);
...
//               System.out.println(String.format("setting %s = %s", header.getKey(), value));
    ...
    public void close() {
        ...
        if (createdHttpClient && httpClient != null) {
            Closeable client = (Closeable) httpClient;
            try {
                client.close();
            } catch (IOException e) {
                logger.debug("Failed to close httpClient", e);
            }
        }
        closed = true;
    }
     ...
     * {@link FileExposingFileEntity} or a regular in-memory {@link org.apache.http.entity.ByteArrayEntity} depending on if the request
